...
import { DAY_END, DAY_START, HOUR_HEIGHT, MIN_DURATION, yToTime, formatHour, numberToTimeInput } from "../../utils/time";
import styles from "./Schedule.module.css";

const hours = Array.from({ length: DAY_END - DAY_START + 1 }, (_, i) => DAY_START + i);

export default function Schedule({ events, selectedDate, onPrevDate, onNextDate, onUpdate, onDelete, onBlankClick }) {
  ...
  const handleEmptyClick = (e) => {
    ...
    onBlankClick?.(numberToTimeInput(start));
  };
        ...
        <div className={styles.grid} ref={gridRef} onClick={handleEmptyClick}>
          ...
          {events.map((event) => (
            <div
              ...
              style={{ top: `${(event.start - DAY_START) * HOUR_HEIGHT}px`, height: `${(event.end - event.start) * HOUR_HEIGHT - 4}px`, borderLeftColor: event.color || "var(--green-700)", background: event.bg || "linear-gradient(90deg,#e6f1e5,#f6f8f2)" }}
              onMouseDown={(e) => startDrag(event, e)}
            >
              <strong>{event.title}</strong>
              {event.subject && <em>{event.subject} · {event.section}</em>}
              <button className={styles.deleteBtn} onClick={(e) => { e.stopPropagation(); onDelete(event.id); }}>×</button>
